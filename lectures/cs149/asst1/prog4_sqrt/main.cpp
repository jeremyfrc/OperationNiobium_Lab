...
int main() {
    ...
    for (unsigned int i=0; i<N; i++)
    {
        ...
        // values[i] = 1.000f; // worst case: all values are the same, so we get no speedup from ISPC but total time much shorter

        // values[i] = 2.998f; // best case: all values are the same, so we get no speedup from ISPC but total time much longer.  Note that the speedup is about 54x.
    }
